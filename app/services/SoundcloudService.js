(function () {
	...
	function SoundcloudService ($http) {
        ...
        vm.explore = function(apiKey ){
            vm.apiKey = apiKey;
            vm.url = 'https://api.soundcloud.com/tracks.json?&client_id=' + vm.apiKey;
            return $http.get(vm.url);
        };

        vm.explorecat = function(apiKey,cat){
            vm.apiKey = apiKey;
            vm.url = 'http://api.soundcloud.com/tracks.json?q='+ cat + '&client_id=' + vm.apiKey;
            return $http.get(vm.url);
        };
	}
